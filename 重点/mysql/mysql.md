...
脑裂产生的影响：客户端不知道数据该写入哪个主节点，结果就是不同的客户端会向不同的主节点上写入数据。当两个主节点恢复网络时，原主库会同新主库做一次全量同步，那么这段时间内原主库写入的数据就会丢失。

脑裂产生的原因：

1.网络分区中的某些节点无法与其他节点进行通信，网络故障、网络拥塞造成的，导致集群中的节点无法进行心跳检测判断其他节点的状态。

2.节点故障，集群中的某些节点发生硬件故障或者软件崩溃，导致节点无法正常工作。

如何解决脑裂：如果主节点不能继续与指定数量的从节点发送心跳，则该节点拒绝写入请求。

### MySQL视图

概念：视图是一个虚拟表，其内容由查询定义。同真实的表一样，视图包含一系列带有名称的列和行数据。但是，数据库中只存放了视图的定义，而没有存放视图中的数据，这些数据存放在原来的表中。使用视图查询数据时，数据库系统会从原来的表中取出相应的数据。因此，数据图中的数据是依赖于原来表中数据的。一旦表中的数据发生改变，显示在视图中的数据也会同步改变。同样对视图的更新也会影响到原来表中的数据。

作用：

1. 使操作简单化

   视图需要达到的目的是所见即所需，视图不仅可以简化用户对数据的理解，也可以简化他们的操作。那些经常使用的查询可以被定义为视图，从而使得用户不必为以后操作每次指定全部条件。

2. 增加数据的安全性

   通过视图，用户只能查询和修改指定的数据。表中出查询字段外的其他数据用户接触不到，这样可以防止敏感信息泄漏，增强安全性。

3. 提高表的逻辑独立性

   视图可以屏蔽原有表结构变化带来的影响。例如原有表增加列和删除未被引用的列，对视图不会造成影响。

视图基本操作

1. 创建视图

   创建视图需要具有`CREATE VIEW`的权限。
...
概念：SQL注入就是恶意用户通过在表单中填写包含SQL关键字的数据来使数据库执行非常规代码的过程。

原因：SQL数据库的操作是通过SQL语句来执行的，而无论是执行代码还是数据项都必须写在SQL语句中，这就导致如果我们在数据项中加入某些SQL语句关键字比如说select,drup等，这些关键字可能在数据库写入或读取数据时得到执行。

SQL注入产生的条件：

1. 参数用户可控：前软传给后端的参数用户可控。
...
创建表：`CREATE Table [IF NOT EXISTS] <数据表名> (字段1 类型 [DEFAULT] [NOT NULL], ... [PRIMARY KEY],[FOREIGEN KEY]);`

删除数据库：`GROP Database [IF EXISTS] <数据库名>;`

修改数据表字段：`ALTER TABLE <数据表名> ALTER <字段> SET DEFAULT <默认值>`

数据表添加字段：`ALTER TABLE <数据表名> ADD <新字段名> <数据类型> [...]`

数据表删除字段`ALTER TABLE <数据表名> DROP <字段名>`

显示数据库创建信息：`SHOW CEATE TABLE <数据库名>`

显示数据表创建信息：`SHOW CREATE TABLE <数据表名>`

查询当前应用哪个数据库:`Select Database();`
...
子查询是一种嵌套在其他sql查询的where子句中的查询，一般用于对查询结果的进一步限制，返回需要的数据。

子查询必须包含在圆括号内，并且不能使用 ORDER BY 进行排序

```sql
select * from student where id in (select student_id from record)
```

##### 内连接

内连接inner join连接两张表，只返回满足on条件的，两张表的交集数据。

```sql
select * from student s inner join record r on s.id=r.student
```

内连接没有通过 on 关键字指定条件的话，查询结果和交叉连接查询结果一样，只是执行效率高于交叉连接

外连接

外连接包含：左连接和右连接

左连接使用关键字`left join`，以左表为准，返回左表的所有数据，右表中满足on条件的数据会全部显示，否则会用null去填充。

右连接使用关键字`right join`，以右表为准。。。。。

##### 完全连接

完全连接使用关键字`full join`，返回左表和右表的所有数据，并用null值填充缺失的数据。

MYSQL不支持完全连接，可以使用左连接+unino+右连接去模拟完全连接
...
交叉连接又称为笛卡尔积，使用关键字cross join连接两张表进行查询。
...
如果给定的是两个参数：第一个参数指定第一个记录行的偏移量，第二个参数指定返回记录的最大数目。初始记录行的偏移量是0，为了检索从某一个偏移量到记录集的结束所有的记录行，可以指定第二个参数为-1.

如果给定的是一个参数，标识返回最大的记录行数目。

```sql
SELECT ... FROM ... WHERE ... ORDER BY ... LIMIT ...  
```

越往后分页，LIMIT语句的偏移量就会越大，速度也会明显变慢。即limit第一个参数越大，速度也越慢。

可以通过子查询的方式来优化分页效率。子查询是在索引上完成的，而普通的查询时在数据文件上完成的，通常来说，索引文件要比数据文件小得多，所以操作起来也会更有效率。
...
Join连接查询：是将两个查询(或表)的每一行，以两两横向对接的方式，得到所有行的结果。即一个表中的某行，跟另一个表中的某行进行横向对接，得到一个新行。

union联合查询：指将2个或2个以上的字段数量相同的查询结果，纵向堆叠后合并为一个结果。
