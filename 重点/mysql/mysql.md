...
3. 并发：InnoDB最小粒度锁为行锁，MyISAM最小粒度锁位表锁。一个更新语句会锁住整个表，导致其他查询和更新都会被阻塞，并发访问受限。
4. 崩溃恢复：InnoDB支持undo log原子性和redo log持久性。在系统掉电崩溃后能够恢复到掉点前的状态。、
5. 备份：InnoDB支持在线热备份，MyISAM不支持。
6. InnoDB是聚簇索引，使用B+树作为索引结构，MyISAM是非聚簇索引。聚簇索引的文件存放在主键索引的叶子节点上，因此InnoDB必须要有主键，通过主键索引效率很高。MyISAM是非聚簇索引，数据文件是分离的，索引保存的是保存文件的指针。
