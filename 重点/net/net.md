...
数据链路层：帧编码和误差纠正控制。将来自物理层流形式的数据封装成帧，将上层帧数据拆分为流（ARP协议）

物理层：解决两个硬件间的通信问题（Ethernet以太网协议）

![img](./assets/v2-1578921092d775e024345fa8a531a85e_b.png)

在 TCP/IP 模式中 ARP 协议属于网络层，在 OSI 参考模型中 ARP 协议属于数据链路层。
...
HTTP依赖TCP进行数据传输，当客户端发送http请求时，它会创建一个tcp连接到服务器的指定端口。tcp连接建立后，http请求和响应的数据通过tcp进行传输。tcp将http消息进行分割，并通过IP协议传输到服务器，服务器端tcp重新组装这些数据，交给http协议进行解析处理。
...
UDP不是面向连接的协议。如果要使用UDP还有保证数据的可靠传输，只能在应用层来实现。具体方法可以参考TCP的可靠传输机制，差别就是将TCP传输层功能，如确认机制、重传机制、流量控制、拥塞控制在应用层实现。

1. 提供超时重传机制，能避免数据报丢失的问题。
2. 提供确认序列号，保证数据拼接时的正确排序。
...
- 服务端和客户端初始化 `socket`，得到文件描述符(listen_fd)；
- 服务端调用 `bind`，将 上一步生成的文件描述符与 IP 地址和端口绑定;
- 服务端调用 `listen`，进行监听；
...
`correspond_fd`：用于通信，有几个连接就对应几个`correspond_fd`文件描述符。

文件描述符对应两块内存区域：读缓冲区和写缓冲区。

调用accept时，查看`listen_fd`的缓冲区，如果有数据，说明有新的客户端连接请求，如果没有数据则阻塞。
...
socket编程中，对于socket的读写默认都是阻塞模式的，有些情况我们需要将其设置为非阻塞，比如多路复用，或者通过select实现连接超时等功能，需要将socket设置为非阻塞。

Windows下使用`ioctlsocket`函数

Linux下使用`fcntl`函数(参数和`ioctlsocket`相似)

`ioctlsocket`：

```cpp
cCopy codeint ioctlsocket(SOCKET s, long   cmd, u_long *argp);
s为要控制的套接字
cmd为要执行的操作
*args为传递给操作的参数。
返回值如果为Socket_ERROR则说明操作失效
```

`ioctlsocket`函数可以用来执行各种操作，比如：

- 获取套接字的当前状态（FIONREAD、FIONBIO）
- 设置套接字的非阻塞模式（FIONBIO） 传输参数为1表示非阻塞   0表示阻塞
- 设置套接字的缓冲区大小（SO_RCVBUF、SO_SNDBUF）
...
详细：发送端每发送一个报文，就启动一个定时器并等待确认信息。接收端成功接收后返回确认西悉尼。若在定时器超时前数据未能被确认，TCP就认为报文中的数据已经丢失或损坏，需要对报文内容进行重新组织和重传。

影响超时重传机制的一个关键参数是重传超时时间（RTO）.RTO的值设置过大或者过小都会对协议造成不利影响。如果RTO设置过大将会使发送端经过较长时间的等待才能发现报文丢失，降低了连接数据传输出的吞吐量。若RTO过小，发送端可能将一些延迟较大的报文认为丢失，造成不必要的重传，浪费了网络资源。
...
- 重传丢失的数据包；
- 如果收到新数据的 ACK 后，把 cwnd 设置为ssthresh 的值，原因是该 ACK 确认了新的数据，说明从 duplicated ACK 时的数据都已收到，该恢复过程已经结束，可以回到恢复之前的状态了，也即再次进入拥塞避免状态.
