...
**协议优化**：RSA密钥协商算法需要4次握手，花费2RTT才能进行数据传输，且不具备前向安全性。
...
HTTP/3基于UDP协议在应用层实现了QUIC协议，具有类似TCP的连接管理、拥塞控制和流量控制，将UDP变为可靠传输。
...
`Close_WAIT`是被动关闭的一方，当被动关闭方没有调用`close`函数关闭连接，那么就无法发送`FIN`，从而使得`Close_WAIT`状态无法转为`LAST_ACK`。
